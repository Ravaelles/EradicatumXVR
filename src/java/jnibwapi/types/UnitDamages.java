...
public class UnitDamages {
	...
	public static void rememberUnitDamageValues() {
		for (UnitTypes unitTypes : UnitTypes.values()) {
			if (!unitTypes.name().startsWith("Terran") && !unitTypes.name().startsWith("Protoss")
			...
			try {
				int groundAttackValue = type.getGroundWeapon().getDamageAmount();
				...
				if (groundAttackValue == 0) {
					...
					double groundAttackValueNormalized = ((double) 30 * groundAttackValue / type
							.getGroundWeapon().getDamageCooldown());
					groundAttackValueNormalized = Double.parseDouble(String.format("%.1f",
							groundAttackValueNormalized, Locale.ENGLISH).replace(',', '.'));
					groundAttackValuesPerSec.put(type, groundAttackValueNormalized);
				}
