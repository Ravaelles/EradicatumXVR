...
import ai.managers.units.workers.GathererManager;
...
public class TerranCommandCenter {
	...
	public static boolean shouldBuild() {
		...
		// Build HQ before Factory
		// if (bases <= 1 && factories > 0 && (battleUnits >=
		// ArmyCreationManager.MINIMUM_MARINES || xvr.canAfford(358))
		// && (TerranBunker.getNumberOfUnitsCompleted() >=
		// TerranBunker.MAX_STACK || xvr.canAfford(384))
		// && factoryFirstConditionOkay) {
		// return ShouldBuildCache.cacheShouldBuildInfo(buildingType, true);
		// }

		// Build Factory before HQ
		if (bases <= 1 && factories >= TerranFactory.MIN_FACTORIES
	...
	private static boolean shouldBuildWorkers(Unit base) {
		...
		if (TerranOffensiveBunker.isStrategyActive()) {
			...
			// Priority for bunker
			if ((TerranBunker.shouldBuild() || TerranBunker.getNumberOfUnits() == 0) && !xvr.canAfford(150)) {
				return false;
			}
		}
	...
	private static void checkNumberOfGasWorkersAt(Unit base) {
		...
		if (gasGatherersForBase > WORKERS_PER_GEYSER) {
			...
			for (int i = 0; i < numRequiredWorkers && i < gatherers.size(); i++) {
				...
				GathererManager.gatherResources(gathererToAssign, base);
			}
		}
	}
	...
	private static void haveOverLimitGasWorkers(Unit base, int overLimitWorkers) {
		...
		// }
		ArrayList<Unit> mineralsInNeihgborhood = xvr.getUnitsOfTypeInRadius(UnitTypes.Resource_Mineral_Field, 25, base,
				false);
		if (!mineralsInNeihgborhood.isEmpty()) {
			for (Unit worker : gatherers) {
				...
				GathererManager.gatherResources(worker, base);
			}
		}
	}
	...
	private static void checkIfRemoveZeroMineralsCrystal(Unit base) {
		...
		if (mineralWorkersNearBase.size() >= ACT_IF_AT_LEAST_N_WORKERS) {
			ArrayList<Unit> mineralsAroundTheBase = xvr.getUnitsOfTypeInRadius(UnitTypes.Resource_Mineral_Field,
			...
			for (Unit mineral : mineralsAroundTheBase) {
				if (mineral.getResources() == 0 && !mineral.isBeingGathered()) {
					...
					GathererManager.forceGatherMinerals(unitToUse, mineral);
				}
			}
		}
	}
	...
	public static MapPoint findTileForNextBase(boolean forceNewSolution) {
		...
		if (nearestFreeBaseLocation != null) {
			...
			_cachedNextBaseTile = Constructing.getLegitTileToBuildNear(BuilderSelector.getRandomWorker(), buildingType,
					point, 0, 10);
	...
	private static int getNumberOfWorkersNearBase(Unit base) {
		return xvr.countUnitsOfTypeInRadius(UnitManager.WORKER, 15, base.getX(), base.getY(), true);
	}
	...
	public static void initialMineralGathering() {
		...
		for (Unit unit : getWorkersNearBase(xvr.getFirstBase())) {
			GathererManager.forceGatherMinerals(unit, minerals.get(counter));
	...
	public static boolean isExistingCompletedAssimilatorNearBase(Unit nearestBase) {
		ArrayList<Unit> inRadius = xvr.getUnitsOfTypeInRadius(TerranRefinery.getBuildingType(), 12, nearestBase, true);
